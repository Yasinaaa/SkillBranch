...
object MarkdownParser {
    ...
    fun clear(string: String?): String? {
        val elements = mutableListOf<Element>()
        elements.addAll(clearRecursion(string!!))
    ...
    private fun clearRecursion(string: CharSequence): List<Element>{
        ...
        loop@ while (matcher.find(lastStartIndex)) {
            ...
            when (group) {
                ...
                3 -> {
                    //text without "> "
                    text = string.subSequence(startIndex.plus(2), endIndex)
                    val subelements = clearRecursion(text)
                    parents.addAll(subelements)
                    lastStartIndex = endIndex
                }
                ...
                6 -> {
                    ...
                    text = string.subSequence(startIndex.plus(2), endIndex.plus(-2))
                    val subelements = clearRecursion(text)
                    //val element = Element.Text(text, subelements)
                    parents.addAll(subelements)
                    lastStartIndex = endIndex
                }
                ...
                9 -> {
                    //full text for regex
                        ...
                        val element = Element.Text(title)
                        parents.add(element)
                        lastStartIndex = endIndex
                }
